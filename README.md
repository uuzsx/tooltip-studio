...
作者：**幼幼紫、千村**。当前版本 **1.3，只支持 Minecraft 1.20.4**，为纯客户端模组。
...
2. 在游戏实例的 `mods/` 中放入 `tooltip-studio-1.3+1.20.4.jar` 和 **适用于 1.20.4 的 Fabric API**。
...
- 每套样式可选填 `offsetX` / `offsetY`，让整个 tooltip 水平或垂直移动；保留屏幕边缘限制。
...
可直接安装的完整示例和详细用法位于 [examples/resource-pack](examples/resource-pack/README.md)：普通木棍/木板使用 pack_wood，Monumenta.Location 为 forest 的物品使用 monumenta/forest，并向上偏移 12 个界面像素。新版示例效果需要 1.3；旧版配置与资源包仍可继续使用。
...
| `offsetX/offsetY` | 可选，各自默认 0；整框在屏幕上的水平/垂直偏移，整数 -4096..4096，单位为 GUI 像素；X 正右负左，Y 正下负上 |
...
## 调整整个 tooltip 的位置（1.3 新增）

在样式 JSON 最外层，与 `texture`、`padding`、`decorations` 同级添加以下两项，保留其他字段：
...
这样会向上移动 12 个界面像素。X 正数向右、负数向左；Y 正数向下、负数向上。两个字段都可省略，也可只写其中一个，未写的值为 0。
边框、背景、名称、分割线、正文、装饰和框内物品预览一起移动，文字与框之间的间距保持不变。字段属于每套样式，适用于本地与资源包 JSON；不写在匹配规则里。
偏移从原来正常显示的位置计算，在自动换行、缩放和鼠标侧边选择之后应用。单位跟随 Minecraft GUI 缩放，不是图片像素或显示器物理像素；tooltip 因过长而整体缩小时，偏移值本身不会再缩小。
最终位置仍保留屏幕边缘 4 个 GUI 像素的空间（含框外装饰），到达边缘时会限制实际移动距离。已占满屏幕高度的长 tooltip 可能无法继续上下移动。
更新本地 JSON 后执行 `/tooltipstudio reload`；更新资源包后按 **F3+T**。如果资源包覆盖了同名本地样式，应修改当前生效的资源包 JSON。
可转发的详细说明见 [examples/offsets](examples/offsets/README.md)。
...
产物在 `build/libs/tooltip-studio-1.3+1.20.4.jar`。模组元数据中的版本为 `1.3`，文件名中的 `+1.20.4` 表示适用的 Minecraft 版本。`-sources.jar` 是源码，不是游戏加载的模组文件。
构建也会生成 `build/resourcepacks/tooltip-studio-example-pack-1.3+1.20.4.zip`，可直接放进游戏的 resourcepacks 文件夹启用。
...
从 mods 文件夹移除旧版 Tooltip Studio JAR，放入 `tooltip-studio-1.3+1.20.4.jar`，同一时间只保留一个版本。已有 JSON 与 PNG 配置无需迁移，未填写 offsetX/offsetY 的样式保持原来位置。需要 NBT 匹配功能时在规则中添加 `nbt`。MixinExtras 0.4.1 已内嵌，无需另外安装；Shulker Box Tooltip 仍是可选模组。
