...
- 新增 Minecraft 1.21.1、1.21.4、26.1.1、26.1.2、26.2、26.3 的独立 NeoForge 构建。已有 Fabric 1.9.0 保持不变。
- 保留居中标题、固定端分割线、条件匹配、资源包扩展、独立/动态图片装饰、分段文字、锚点、缩放及鼠标相对偏移。内置素材仍只有默认款与模组图标。
- 旧 nbt 条件读取 minecraft:custom_data；components 使用对应游戏版本的持久化 codec，支持其他模组注册的组件和世界注册表。
- 按版本适配 NBT API、即时/延后 GUI 绘制、tooltip 组件图片、26.3 资源包覆盖层及新版模组图标元数据。
- 为六个目标生成对应资源包格式和 /give 文本语法的七类示例；增加独立 Gradle 工程、六版本 CI 和客户端集成测试。

# 1.9.0

- 新增 Minecraft 1.21.1、1.21.2、1.21.3、1.21.4 的 Fabric 构建，每个游戏版本单独提供 JAR；同时保留 1.20.4。
- 新版中的旧 nbt 条件、TooltipStyle 从 minecraft:custom_data 读取，自定义字段路径无需加组件前缀；保留物品 ID、tag、稀有度及既有规则优先级。
- 样式规则和独立装饰规则新增 components 条件，可匹配原版及其他模组注册的可持久化组件。支持标量、候选数组和组件内部路径，多项条件共同生效；无效配置保留上一次成功状态。
- 适配新版 tooltip 渲染、收纳袋与 Shulker Box Tooltip 预览 API，保留动态图片、独立贴图、多段彩色文字、锚点、缩放、偏移及资源包热重载。
- 以共享源码和少量版本适配层维护五个构建目标，新增组件资源包示例与迁移说明；新版示例包使用对应 pack_format 和组件格式的演示命令。
