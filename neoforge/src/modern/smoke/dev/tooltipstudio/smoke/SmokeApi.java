...
final class SmokeApi {
    ...
    static boolean ready(Minecraft mc) {
        if (dev.tooltipstudio.TooltipStudioClient.CONFIG.count() == 0 || mc.getOverlay() != null) return false;
        if (!worldRequested) {
            worldRequested = true;
    ...
    static void capture(Minecraft mc, String name, java.util.function.Consumer<Component> callback) { Screenshot.grab(mc.gameDirectory, name, mc.getMainRenderTarget(), 1, callback); }
