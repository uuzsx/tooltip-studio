...
public final class RenderApi {
    ...
    private static final class Canvas implements TooltipCanvas {
        ...
        public void componentImage(ClientTooltipComponent c,int x,int y,int w,int h) { c.renderImage(font,x,y,w,h,graphics); }
        public void text(Component text,int x,int y,int color,boolean shadow) { graphics.drawString(font,text,x,y,color,shadow); }
        public void texture(ResourceLocation id,int x,int y,int w,int h,float u,float v,int sw,int sh,int tw,int th) { graphics.blit(RenderType::guiTextured,id,x,y,u,v,w,h,sw,sh,tw,th); }
    }
}
