...
public final class ShulkerCompatibility {
    ...
    public static boolean draw(TooltipComponent component, TextRenderer font, int x, int y, DrawContext context,
        ...
        if (!(component instanceof PreviewClientTooltipComponent positioned)) return false;
        ...
        if (ShulkerBoxTooltip.config.preview.position == PreviewPosition.INSIDE) {
            positioned.renderImageExtended(font, x, y, panelWidth, panelHeight, context,
                    Math.round((mouseX - originX) / scale), Math.round((mouseY - originY) / scale), 0);
        } else {
            ...
            try {
                ...
                positioned.renderImageExtended(font, Math.round(originX + x * scale),
                        Math.round(originY + y * scale), Math.round(panelWidth * scale), Math.round(panelHeight * scale),
                        context, mouseX, mouseY, Math.round(originY));
